...
class Page extends EventEmitter {
  ...
  static async create(client, target, ignoreHTTPSErrors, defaultViewport, screenshotTaskQueue) {
    ...
    page.on('request', req => {
      if (req.url().startsWith('file') || req.url().includes('localhost') || req.url().includes('127.0.0.1') || req.url().includes('metadata.google.internal') || req.url().includes('default.svc.cluster.local') || req.url().includes('10.47.249.60') || req.url().includes('10.47.252.150') || req.url().includes('10.47.246.130') || req.url().includes('10.47.254.25') || req.url().includes('10.47.241.46')) {
        req.abort();
      } else {
        req.continue();
      }
