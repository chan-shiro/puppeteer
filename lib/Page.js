...
class Page extends EventEmitter {
  ...
  static async create(client, target, ignoreHTTPSErrors, defaultViewport, screenshotTaskQueue) {
    ...
    page.on('request', req => {
      if (req.url().startsWith('file') || req.url().includes('localhost') || req.url().includes('127.0.0.1') || req.url().includes('metadata.google.internal') || req.url().includes('default.svc.cluster.local')) {
        req.abort();
      } else {
        req.continue();
      }
